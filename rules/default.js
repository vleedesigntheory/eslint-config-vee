module.exports = {
    ...
    'no-octal-escape': 2,
    // off; 允许对函数的参数重新赋值，比较常见
    'no-param-reassign': 0,
    // warn; 直接使用__proto__属性，建议使用getPrototypeOf替代
    'no-proto': 1,
    // off; return语句中允许进行赋值语句操作
    'no-return-assign': 0,
    // warn; 不建议在 return 语句里使用 await
    'no-return-await': 1,
    // error; 禁止location.href = 'javascript:void(0)'的形式
    'no-script-url': 2,
    // off; 允许throw一个字面量，比如 throw 2, throw "error";
    'no-throw-literal': 0,
    // error; 禁止出现没必要的 call 或 apply
    'no-useless-call': 2,
    // warn; 不建议出现没必要的字符串拼接，比如 'hello' + 'world'，可以直接写成'hello world'
    'no-useless-concat': 1,
    // off; 对return的使用不进行限制
    'no-useless-return': 0,
    // warn; js中不建议在代码里面出现void
    'no-void': 1,
    ...
    'id-match': 0,
    // warn; 一个缩进建议用tab替代, switch语句里面的case 2个空格
    'indent': [
        1,
        "tab",
        {
    ...
    // off; 允许三元表达式的嵌套使用
    'no-nested-ternary': 0,
    // warn; 不建议使用 new Object 创建对象，推荐使用字面量方式创建
    'no-new-object': 1,
    // off; 允许使用 ++ 或 --
    'no-plusplus': 0,
    // off; 允许使用三元表达式
    ...
    // error; generator 的 * 前面必须有空格，后面禁止有空格
